		...
		exports.findUserThread = function(req, res) {
			...
			messageObj.find({
				...
				$or: [{
					...
					recieverid: req.params.contactId
				}],
				is_deleted: false,
				message: {
					$elemMatch: {
			...
			messageObj(messageModelObj).save(req.body, function(err, data) {
				...
				if (err) {
					...
					outputJSON = {
						'status': 'success',
						'messageId': 200,
						'message':req.headers.lang=="chtrad" ? chtradconstantObj.messages.successRetreivingData : (req.headers.lang=="chsimp" ? chsimpconstantObj.messages.successRetreivingData : constantObj.messages.messageSuccess) ,
						data: data
					};
					res.jsonp(outputJSON);
					...
					messageObj.load(req.params.messageId, function(err, message) {
						if (err) {
							...
							outputJSON = {
								'status': 'success',
								'messageId': 200,
								'message': constantObj.messages.messageStatusUpdateSuccess,
								message: message
							};
							res.jsonp(outputJSON);
						}
